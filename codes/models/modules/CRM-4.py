...
class CRM(nn.Module):
    def __init__(self, n_feats=32, hidden_list=[256, 256]) -> None:
        ...
        self.routing = MLP(in_dim=self.n_feats, out_dim=4, hidden_list=[256])
    ...
    def resample(self, feat, coord, cell):
        ...
        for i in range(4):
            preds[i] = torch.mul(preds[i], routing_weights[:, :, i].unsqueeze(-1))
        for pred, area in zip(preds, areas):
            res = res + pred * (area / tot_area).unsqueeze(-1)
